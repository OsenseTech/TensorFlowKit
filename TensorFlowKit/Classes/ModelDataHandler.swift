...
enum MobileNet {
  static let modelInfo: FileInfo = (name: "tiny_ep499_576_320", extension: "lite")
...
class ModelDataHandler {
  ...
  private func rgbDataFromBuffer(_ buffer: CVPixelBuffer, byteCount: Int, isModelQuantized: Bool) -> Data? {
    ...
    let bytes: UnsafeRawPointer = UnsafeRawPointer(destinationBuffer.data)
    let count: Int = destinationBuffer.rowBytes * height
    let byteData = Data(bytes: bytes, count: count)
    ...
    let byte = Array<UInt8>(unsafeData: byteData)!
    var floats = [Float]()
    for i in 0..<byte.count {
        floats.append(Float(byte[i]) / 255.0)
    }
